...
import (
	...
	ptg "github.com/joincloud/peers-touch-go/pubsub"
...
func main() {
	...
	ch, err := n.Broker().Join(ctx,
		ptg.ChannelPubsub(ps),
		ptg.ChannelPeerID(h.ID()),
		ptg.ChannelMetadata(
			map[string]string{"nickName": nick}),
		ptg.ChannelCodec(n.Broker().Codec()),
		ptg.ChannelName(room))
	if err != nil {
		panic(err)
	}
